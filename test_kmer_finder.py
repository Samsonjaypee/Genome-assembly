...
class TestFindSubstrings:
    def test_find_substrings_true_1(self):
        sequence = "ATCGATCG"
        k = 3
    ...
    def test_find_substrings_true_2(self):
        sequence = "ATCGATCG"
        k = 2
        result = find_substrings(sequence, k)
        assert result == {'AT': ['TC', 'TC'], 'TC': ['CG', 'CG'], 'CG': ['GA', 'G'], 'GA': ['AT']}

    def test_find_substrings_false_1(self):
        sequence = "ATCGATCG"
        k = 3
        result = find_substrings(sequence, k)
        assert result != {'ATC': ['TCG'], 'TCG': ['CGA', 'C']}

    def test_find_substrings_false_2(self):
        sequence = "ATCGATCG"
        k = 2
        result = find_substrings(sequence, k)
...
class TestFindSubstringsFromFile:
    def test_find_substrings_from_file_true_1(self):
        filename = "test_files/test_sequences1_true.fasta"  # Provide a file with appropriate sequences for testing
        k = 3
    ...
    def test_find_substrings_from_file_false_1(self):
        filename = "test_files/test_sequences3_false.fasta"  # Provide a file with inappropriate sequences for testing
        k = 3
    ...
    def test_find_substrings_from_file_false_2(self):
        filename = "test_files/test_sequences4_false.fasta"  # Provide a file with inappropriate sequences for testing
        k = 2
        with pytest.raises(ValueError, match="Invalid character in sequence. Only 'A', 'T', 'C', 'G', and 'N' are allowed."):
            find_substrings_from_file(filename, k)

    def test_find_substrings_from_file_invalid_file(self):
        filename = "nonexistent_file.txt"  # Provide a non-existent file name
        k = 3
...
class TestFindSmallestK:
    def test_find_smallest_k_true_1(self):
        filename = "test_files/test_sequences1_true.fasta"  # Provide a file with appropriate sequence fragments for testing
        result = find_smallest_k(filename)
        assert result == 1

    def test_find_smallest_k_true_2(self):
        filename = "test_files/test_sequences2_true.fasta"  # Provide a file with appropriate sequence fragments for testing
        result = find_smallest_k(filename)
        assert result == 8
